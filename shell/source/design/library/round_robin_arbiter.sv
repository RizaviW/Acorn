module round_robin_arbiter #(
  ...
  logic   [BITS-1:0] current_priority;
  logic [2*BITS-1:0] next_priority;
  // Logic
  always_comb begin
    double_request = {request, request};
    double_grant = double_request & ~(double_request - current_priority);
  ...
  always_ff @(posedge clock) begin
    if (reset) begin
      ...
      current_priority <= request ? next_priority[2*BITS-1:BITS] | next_priority[BITS-1:0] : current_priority;
    end
  end
endmodule
