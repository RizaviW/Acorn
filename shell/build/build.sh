...
cat <<EOT > "$BUILD_TCL"
    ...
    set sources [list "$SRC_DIR/design/shell.sv" "$SRC_DIR/design/library/DPRAM.sv"]
    set filelist "$SRC_DIR/design/$PROJECT/filelist.f"
    if {[file exists \$filelist]} { lappend sources {*}[exec envsubst < \$filelist | awk {NF && !/^([#+]|\/\/)/}] }
